...
/**
 * Parses the Single Line (SING) CLOB records.
 * 1. Retrieves the value associated with the "propertyTN" key from the OMS CLOB.
 * 2. Retrieves the value associated with the "listingType" key from the OMS CLOB.
 *
 * @author Damian Etarsky
 */
public class SingleLineClobParser {
   ...
   // used by the OMS datasource to indicate that a WTN is publicly listed
   private static final String LISTED = "LIS";

   // used by the OMS datasource to indicate that a WTN is NOT publicly listed
   private static final String NON_LISTED = "NONPUB";
    ...
    * @param parameterTwo specifies one of the keys to be parsed in the OMS clob structure
    * @param parameterThree specifies an additional parameter to pass to the parser
    ...
    */
   public SingleLineClobParser(String inputFilename, String outputFilename, String parameterOne,
                               String parameterTwo, String parameterThree, int bufferSize, int fieldSize,
                               String clobDelimiter, String propertyDelimiter)
           throws Exception
   {
   ...
   private void execute() throws IOException
   {
      ...
      while((line = m_input.readLine()) != null) {
         ...
         String valueOne = parse(line, m_parameterOne);
         buffer.append(Utils.padleft(valueOne, m_fieldSize));

         /* append parameter three to the buffer */
         buffer.append(Utils.padleft(m_parameterThree, m_fieldSize));

         /* parse the value associated with parameter two from the clob */
         String valueTwo = parse(line, m_parameterTwo);
         if(valueTwo.equals(LISTED)) {
            buffer.append("N");
         }
         else if(valueOne.equals(NON_LISTED)) {
            buffer.append("Y");
         }
         else {
            buffer.append(" ");
    ...
    *
    * Goal: parse the value associated with the primaryTN property name
    * Input: primaryTN+value;listingType+value;key(n)+value(n);
    * Output: the value associated with the primaryTN property name
    *
