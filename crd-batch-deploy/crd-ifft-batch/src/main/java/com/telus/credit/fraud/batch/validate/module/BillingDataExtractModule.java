 ...
 * Time: 2:38:47 PM
 */
public class BillingDataExtractModule implements DataExtractModule {

   private static final Log log = LogFactory.getLog(BillingDataExtractModule.class);

   // holds the path to the data extract file
   private static final String m_extractFilename = DataExtractModule.PATH + "CUSTODS_REF_BILLING_ACCOUNT.DAT";

   // holds the path to the audit file
   private static final String m_auditFilename = DataExtractModule.PATH + "CUSTODS_REF_BILLING_ACCOUNT.AUD";
   ...
   {
      log.info(" **** BillingDataExtractModule.execute: validating the billing extract file. ****");
      ...
      try {
         ...
         throw new InvalidDataExtractException("BillingDataExtractModule: the extract file is invalid, see stack trace for errors.", e);
      }
      catch (InvalidAuditFileException e) {
         throw new InvalidAuditFileException("BillingDataExtractModule: the audit file is invalid (COD.AP481.001 - IA Customer ODS - Billing Account v2.1.doc) see stack trace for errors.", e);
      }
      finally {
         reader.close();
      }
