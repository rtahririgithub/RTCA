...
import com.telus.crd.assessment.batch.domain.OrderProduct;
 ...
 * @author x136675
 */
public class SbiDepositCalcOrderProcessor implements Module
{
    ...
    {
        OrderProduct ic = transform( m_inputStr, DELIM );
        if ( ic.getOrderDepId().equalsIgnoreCase( currentID ) ) {
            m_pWriter.print( DELIM + ic.getReqRentedEquipCount() + DELIM + ic.getPurchasedCount() + DELIM + ic.getRentedEquipCount() );
        }
        else {
            if ( !currentID.equalsIgnoreCase( "" ) ) m_pWriter.println();

            currentID = ic.getOrderDepId();
            m_pWriter.print( ic.getOrderDepId() + DELIM + ic.getServiceTypCd() + DELIM + ic.getReqRentedEquipCount() + DELIM + ic.getPurchasedCount() + DELIM + ic.getRentedEquipCount() );
        }
    ...
    private OrderProduct transform(String strLine,
            String delimiter)
    {
        ...
        OrderProduct ic = new OrderProduct();
        for (int i = 0 ; i < splitStr.length ; i++) {
            String value = splitStr[i];
            
            if ( i == 0 ) {
                ic.setOrderDepId( value );
            }
            else if ( i == 1 ) {
                ic.setServiceTypCd( value );
            }
            else if ( i == 2 ) {
                ic.setReqRentedEquipCount( value );
            }
            else if ( i == 3 ) {
                ic.setPurchasedCount( value );
            }
            else if ( i == 4 ) {
                ic.setRentedEquipCount( value );                
            } else {
                break;
            }
        }
