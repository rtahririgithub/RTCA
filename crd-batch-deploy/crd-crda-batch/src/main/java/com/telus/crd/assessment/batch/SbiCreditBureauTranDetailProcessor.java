 ...
 * @author x136675
 */
public class SbiCreditBureauTranDetailProcessor implements Module
{
    ...
    public void execute() throws ModuleException
    {
        ...
        if ( cbtd.getCarId().equalsIgnoreCase( currentID ) ) {
            m_pWriter.print( DELIM + cbtd.getCode() + DELIM + cbtd.getVal() );
        }
        else {
            if ( !currentID.equalsIgnoreCase( "" ) ) m_pWriter.println();

            currentID = cbtd.getCarId();
            m_pWriter.print( cbtd.getCarId() + DELIM + cbtd.getCode() + DELIM
                    + cbtd.getVal() );
        }
