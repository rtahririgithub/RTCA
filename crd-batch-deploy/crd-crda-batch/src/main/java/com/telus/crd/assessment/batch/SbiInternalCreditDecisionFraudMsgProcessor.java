 ...
 * @author x136675
 */
public class SbiInternalCreditDecisionFraudMsgProcessor implements Module
{
    ...
    {
        InternalCreditDecisionTransaction ic = transformDetail( m_inputStr, DELIM );
        if ( ic.getCarId().equalsIgnoreCase( currentID ) ) {
            m_pWriter.print( DELIM + ic.getFraudMsgCd() );
        }
        else {
            if ( !currentID.equalsIgnoreCase( "" ) ) m_pWriter.println();

            currentID = ic.getCarId();
            m_pWriter.print( ic.getCarId() + DELIM + ic.getFraudMsgCd() );
        }
