...
# Filename: rtca_pre_cods_billing_7_less_year.sh
...
if [ $# -eq 2 ]; then
    ...
    echo ""
    echo "usage:rtca_pre_cods_billing_7_less_year.sh  [input file1] [output file} " 
...
syncsort << EOF
    ...
    /CONDITION filter ((BAN_STATUS="O") or ((BAN_STATUS="C") and (STATUS_DT>TODAY))) 
    /INCLUDE filter 
