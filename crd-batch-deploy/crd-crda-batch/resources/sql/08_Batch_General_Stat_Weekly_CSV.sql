/*
    GENERAL STAT WEEKLY [2010/08/12 - for review]
...
SELECT '01' SORT_ORDER, 'Report Name        General Stat (Weekly)' from dual
UNION
SELECT '02' SORT_ORDER, 'Date               ' || TO_CHAR(SYSDATE, 'DD/MM/YYYY') from dual
UNION
SELECT '03' SORT_ORDER, 'Period Covered     ' || TO_CHAR(TRUNC(SYSDATE-7,'D'), 'DD/MM/YYYY') || ' - ' || TO_CHAR(TRUNC(SYSDATE-1,'D'), 'DD/MM/YYYY') from dual
UNION
...
WHERE 
    ...
    AND car_status.EFF_START_TS BETWEEN TRUNC(SYSDATE-7,'D') and TRUNC(SYSDATE,'D')
UNION
SELECT
    '01' SORT_ORDER, 
    'COMPLETED' ||'~'||
...
WHERE car_status.EFF_START_TS BETWEEN TRUNC(SYSDATE-7,'D') and TRUNC(SYSDATE,'D')
