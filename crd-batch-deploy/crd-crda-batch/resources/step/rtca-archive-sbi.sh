...
#Create SBI directory from archive
if [ -d "$3" ];
then
	echo "directory(${3}) already exists"
else
	echo "write directory name ${3}"
	mkdir ${3}
fi
...
echo find $3/. -mtime +365 \( ! -name . -prune \) -type f \( -name "*.gz*" \) -exec rm -f {}
find $3/. -mtime +365 \( ! -name . -prune \) -type f \( -name "*.gz" \) -exec rm -f {} \;
echo find $4/. \( ! -name . -prune \) -type f \( -name "*.dat" \) -exec rm -f {} \;
